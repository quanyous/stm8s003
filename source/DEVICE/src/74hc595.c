...
void displaytext(void)
{
	...
	SPI_SendData(0x08);Delay1();
	while(SPI_GetFlagStatus(SPI_FLAG_TXE)==RESET);
	SPI_SendData(0x04);Delay1();
	while(SPI_GetFlagStatus(SPI_FLAG_TXE)==RESET);
	SPI_SendData(0x02);Delay1();
	while(SPI_GetFlagStatus(SPI_FLAG_TXE)==RESET);

	GPIO_WriteHigh( STCP_GPIO_PORT, STCP_GPIO_PIN );
...
{
	u8 i = 0;
	GPIO_WriteLow( STCP_GPIO_PORT, STCP_GPIO_PIN );
...
void LED_ShowOneToOne(void)	
{
		...
		u8 Waterfall_Led[24] = {0x01, 0x02, 0x04, 0x08, 0x10,0x20,0x40,0x80,  //控制第一级74HC595	
		 ...
		 {					
			Led_Show[0] = Waterfall_Led[pos];			//存放第一级74HC595数据，因为先进先出，所以第一级放在Led_Pos_Buf[1]，而不是Led_Pos_Buf[0]
			Led_Show[1] = Waterfall_Led[pos+8]; 	//存放第二级74HC595的数据	
            Led_Show[2]	= Waterfall_Led[pos+16]	;    //第三级		
		...
		for (pos = 16; pos < 24; pos ++) 				//第pos个灯，实现流水灯效果
		 {					
			Led_Show[0] = Waterfall_Led[pos];			//存放第一级74HC595数据，因为先进先出，所以第一级放在Led_Pos_Buf[1]，而不是Led_Pos_Buf[0]
			Led_Show[1] = Waterfall_Led[pos]; 	//存放第二级74HC595的数据	
            Led_Show[2]	= Waterfall_Led[pos-16]	;    //第三级		
			HC595_Send_Multi_Byte(Led_Show,3);		//将当前数据发送到595	 
